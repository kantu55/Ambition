...
namespace Ambition.GameCore
{
    ...
    public class GameTurnManager : MonoBehaviour
    {
        ...
        private async UniTask ShowEventPhaseAsync()
        {
            ...
            {
                var scheduledEvent = DataManager.Instance.GetDatas<EventMaster>()
                    .FirstOrDefault(e => e.EventId == scheduledEventId);
                if (scheduledEvent != null)
                {
                    Debug.Log($"[GameTurnManager] スケジュール済みイベント発生: {scheduledEvent.Title} (EventId:{scheduledEvent.EventId}) 月:{currentMonth}");
                }
            }
